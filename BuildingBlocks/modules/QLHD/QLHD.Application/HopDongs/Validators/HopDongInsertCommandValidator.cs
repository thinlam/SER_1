...
{
    public HopDongInsertCommandValidator(IServiceProvider serviceProvider)
    {
        ...
        RuleFor(x => x.Model.SoHopDong)
            ...
            .MustAsync(async (soHopDong, cancellationToken) =>
            {
                ...
                return !await repository.GetQueryableSet()
                    .AnyAsync(h => h.SoHopDong == soHopDong, cancellationToken);
            })
        ...
        // DuAnId is optional - only validate if provided
        RuleFor(x => x.Model.DuAnId)
            .MustAsync(async (duAnId, cancellationToken) =>
            {
                if (!duAnId.HasValue || duAnId.Value == Guid.Empty) return true; // Optional, skip if null or empty

                var duAnRepository = serviceProvider.GetRequiredService<IRepository<DuAn, Guid>>();
                return await duAnRepository.GetQueryableSet()
                    .AnyAsync(d => d.Id == duAnId.Value, cancellationToken);
            })
            .WithMessage("Dự án không tồn tại.");

        // Validate DuAnId is not already assigned to another HopDong (one-to-one relationship)
        RuleFor(x => x.Model.DuAnId)
            .MustAsync(async (duAnId, cancellationToken) =>
            {
                if (!duAnId.HasValue || duAnId.Value == Guid.Empty) return true; // Optional, skip if null or empty
                ...
                var repository = serviceProvider.GetRequiredService<IRepository<HopDong, Guid>>();
                return !await repository.GetQueryableSet()
                    .AnyAsync(h => h.DuAnId == duAnId.Value, cancellationToken);
            })
            .WithMessage("Dự án này đã có hợp đồng. Mỗi dự án chỉ được liên kết với một hợp đồng.");
