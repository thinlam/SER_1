...
public class DuAnInsertCommandValidator : AbstractValidator<DuAnInsertCommand>
{
    public DuAnInsertCommandValidator(IServiceProvider serviceProvider)
    {
        ...
        // Validate TrangThaiId belongs to KHOACH type when provided
        RuleFor(x => x.Model.TrangThaiId)
            .MustAsync(async (trangThaiId, cancellationToken) =>
            {
                if (trangThaiId == null) return true; // Skip if null - default will be used
                ...
                return await repository.GetQueryableSet()
                    .AnyAsync(t => t.Id == trangThaiId.Value && t.MaLoaiTrangThai == LoaiTrangThaiConstants.KeHoach, cancellationToken);
            })
